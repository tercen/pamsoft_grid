...
%     error('PamSoft_Grid property ''grdCol'' has not been set');
    exitCode = -1003;
    pg_error_message('grid.preprocess.col_unset', exitCode);
    return
end
...
%     error('PamSoft_Grid property ''grdIsReference'' has not been set');
    exitCode = -1004;
    pg_error_message('grid.preprocess.row_unset', exitCode);
    return
end
...
% @TODO Iseg and Igrid must be the same image (for gridding and
% segmentation), but it has not been defined yet how this will be done
switch imageUse
    case 'Last'
        Igrid = I(:,:,bLast);
        if sum(bLast) > 1
